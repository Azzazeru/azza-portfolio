...
export default function CertificationsCatalog() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden px-4 py-16 lg:pb-24 text-white">
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        ...
        <div className="lg:px-4">
          ...
          <section className="mt-4 grid auto-rows-fr grid-cols-1 gap-4 sm:grid-cols-2 lg:mt-6 lg:grid-cols-3 xl:grid-cols-4">
            {items.map((cert) => (
              <article
                key={`${cert.slug}-${cert.sort_order}`}
                className="group flex h-full min-h-[19.5rem] flex-col rounded-xl border border-white/10 bg-gradient-to-b from-slate-900/90 via-slate-900/90 to-black/90 p-5 shadow-lg shadow-black/20 transition duration-300 hover:-translate-y-1 hover:border-green-300/50 hover:shadow-[0_16px_40px_rgba(34,197,94,0.18)]"
              >
                <div className="mb-3 flex h-28 items-center justify-center rounded-lg border border-white/10 bg-black/90 p-3 transition group-hover:border-green-300/30">
                  {cert.image ? (
                    <Image
                      src={cert.image}
                      alt={cert.name}
